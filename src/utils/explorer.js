...
window.onload = () => {
  showSplash(); // Just check if we need to show the login splash on boot
  
  // Optional: If auth is already active from a previous session, load the root
  if (sessionStorage.getItem("vaultage_auth_active")) {
...
window.executeLoginFlow = async function () {
  ...
  try {
    await window.login(); // This calls /auth/login and sets the cookie

    sessionStorage.setItem("vaultage_auth_active", "true");

    // NOW load the root content since we are authenticated!
    runTest("/folders/content", "RESOLVE_ROOT");
